...
func WriteVarUintAddress(w io.Writer, value uint64) error {
	if err := WriteVarBytes(w, BigIntToNeoBytes(big.NewInt(int64(value)))); err != nil {
...
func WriteAddress(w io.Writer, address Address) error {
	if err := WriteVarBytes(w, address[:]); err != nil {
		return fmt.Errorf("serialize value error:%v", err)
	}
	return nil
}
...
func ReadAddress(r io.Reader) (Address, error) {
	...
	return AddressParseFromBytes(from)
}
...
func EncodeVarUint(sink *ZeroCopySink, value uint64) (size uint64) {
	return sink.WriteVarBytes(BigIntToNeoBytes(big.NewInt(int64(value))))
}
...
func DecodeAddress(source *ZeroCopySource) (Address, error) {
	...
	if irregular {
		return Address{}, ErrIrregularData
	}
